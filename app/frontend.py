...
def display_recommendations(recommendations):
    st.markdown("### おすすめのクラブセット")
    
    # メインの推奨クラブセット
    with st.container():
        display_card("推奨クラブセット", "あなたに最適なクラブセットです", "🏆")
        
        col1, col2 = st.columns([1, 2])
        with col1:
            st.image(recommendations["main_set"]["image_url"], use_column_width=True)
        with col2:
            st.markdown(f"""
            <div style='margin: 1rem 0;'>
                <h3 style='color: #2c5282;'>{recommendations["main_set"]["name"]}</h3>
                <p style='font-size: 1.2rem;'>¥{recommendations["main_set"]["price"]:,}</p>
                <p>{recommendations["main_set"]["description"]}</p>
            </div>
            """, unsafe_allow_html=True)
            
            # 特徴の表示
            st.markdown("#### 特徴")
            for feature in recommendations["main_set"]["features"]:
                st.markdown(f"- {feature}")
    
    # 代替オプション
    st.markdown("### 代替オプション")
    cols = st.columns(3)
    for i, option in enumerate(recommendations["alternatives"]):
        with cols[i]:
            display_card(option["name"], f"¥{option['price']:,}", "🔍")
            st.image(option["image_url"], use_column_width=True)
            st.markdown(f"""
    ...
    # フィッティング分析
    st.markdown("### フィッティング分析")
    with st.container():
        display_card("分析結果", "あなたのスイングに基づく分析結果です", "📊")
        
        # スイング分析
        st.markdown("#### スイング分析")
        for analysis in recommendations["fitting_analysis"]["swing_analysis"]:
            st.markdown(f"- {analysis}")
        
        # クラブ選択の理由
        st.markdown("#### クラブ選択の理由")
        for reason in recommendations["fitting_analysis"]["club_selection_reasons"]:
            st.markdown(f"- {reason}")
        
        # 改善提案
        st.markdown("#### 改善提案")
        for suggestion in recommendations["fitting_analysis"]["improvement_suggestions"]:
...
def display_header():
    st.markdown("""
    <div class='fade-in' style='text-align: center; padding: 2rem 0; background: linear-gradient(135deg, #2c5282, #1a365d); color: white; margin-bottom: 2rem; border-radius: 0 0 20px 20px;'>
        <h1 style='margin-bottom: 1rem; font-size: 2.5rem;'>ぴったりゴルフ</h1>
        <p style='font-size: 1.2rem; opacity: 0.9;'>あなたに最適なゴルフクラブを見つけましょう</p>
    </div>
    """, unsafe_allow_html=True)

# カスタムカードコンポーネント
def display_card(title: str, content: str, icon: str = None):
    st.markdown(f"""
...
def display_user_input_form():
    ...
    with st.container():
        ...
        with col1:
            display_card("基本情報", "身長、体重、年齢などの基本情報を入力してください", "👤")
            height = st.number_input("身長 (cm)", min_value=140, max_value=220, value=170, step=1, key="height")
            weight = st.number_input("体重 (kg)", min_value=30, max_value=150, value=65, step=1, key="weight")
            age = st.number_input("年齢", min_value=10, max_value=100, value=30, step=1, key="age")
            gender = st.selectbox("性別", ["男性", "女性"], key="gender")
            handedness = st.selectbox("利き手", ["右", "左"], key="handedness")
        
        with col2:
            display_card("ゴルフ情報", "ゴルフ歴やハンディキャップなどの情報を入力してください", "🏌️")
            handicap = st.number_input("ハンディキャップ", min_value=0, max_value=54, value=20, step=1, key="handicap")
            golf_years = st.number_input("ゴルフ歴 (年)", min_value=0, max_value=50, value=3, step=1, key="golf_years")
            play_frequency = st.selectbox(
                "プレイ頻度",
                ["週1回以上", "月2-3回", "月1回", "数ヶ月に1回"],
                key="play_frequency"
            )
    ...
    with st.container():
        ...
        with col1:
            display_card("スイングデータ", "ヘッドスピードやボールスピードなどのデータを入力してください", "📊")
            head_speed = st.number_input(
                "ヘッドスピード (m/s)",
                min_value=20.0,
                max_value=60.0,
        ...
        with col2:
            display_card("スイングの悩み", "現在のスイングで気になっている点を選択してください", "❓")
            swing_issues = st.multiselect(
                "スイングの悩み（複数選択可）",
                ["スライス", "フック", "トップ", "ダフリ", "飛距離不足", "方向性の不安定さ", "その他"],
                key="swing_issues"
            )
            if "その他" in swing_issues:
                other_issue = st.text_input("具体的な悩みを入力してください", key="other_issue")
    
    # 予算設定
    st.markdown("### 予算設定")
    with st.container():
        display_card("予算", "クラブセットの予算を設定してください", "💰")
        budget = st.slider(
            "予算 (万円)",
            min_value=10,
            max_value=50,
        ...
        <div style='text-align: center; margin: 1rem 0;'>
            <span style='font-size: 1.5rem; color: #2c5282;'>¥{budget:,}万円</span>
        </div>
        """, unsafe_allow_html=True)
    ...
    with col2:
        if st.button("最適なクラブを検索", key="search_button", use_container_width=True):
            # 入力データの収集
            user_data = {
                "height": height,
                "weight": weight,
            ...
            # プログレスバーの表示
            with st.spinner("最適なクラブを検索中..."):
                # API呼び出し
                try:
                    response = requests.post("http://localhost:8000/api/recommend", json=user_data)
                    if response.status_code == 200:
                        recommendations = response.json()
                        return recommendations
                    else:
                        st.error("クラブの検索に失敗しました。もう一度お試しください。")
                except Exception as e:
                    st.error(f"エラーが発生しました: {str(e)}")
...
def main():
    ...
    # メインコンテンツ
    if menu_option == "クラブ検索":
        # ユーザー入力フォームの表示
        recommendations = display_user_input_form()
    ...
    elif menu_option == "フィッティング履歴":
        st.markdown("### フィッティング履歴")
        # TODO: フィッティング履歴の表示機能を実装
